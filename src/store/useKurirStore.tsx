...
const STORAGE_KEY = 'bungkurir_data_v3';
...
interface KurirContextType {
  ...
  setActiveJadwalId: (id: string | null) => void;
  createJadwal: (name: string, location?: { latitude: number; longitude: number; address: string }) => Jadwal;
  deleteJadwal: (id: string) => void;
  addBuyer: (jadwalId: string, buyerData: Pick<Buyer, 'name' | 'address' | 'latitude' | 'longitude'>) => void;
  updateBuyerInfo: (jadwalId: string, buyerId: string, info: Partial<Buyer>) => void;
  updateBuyerStatus: (jadwalId: string, buyerId: string, status: DeliveryStatus, data: any) => void;
  deleteBuyer: (jadwalId: string, buyerId: string) => void;
  isHydrated: boolean;
}

const KurirContext = createContext<KurirContextType | undefined>(undefined);

export function KurirProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <KurirContext.Provider value={{
      ...
      {children}
    </KurirContext.Provider>
  );
}

export function useKurirStore() {
  const context = useContext(KurirContext);
  if (context === undefined) {
    throw new Error('useKurirStore must be used within a KurirProvider');
  }
  return context;
}
